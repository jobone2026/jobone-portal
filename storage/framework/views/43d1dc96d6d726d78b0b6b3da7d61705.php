    ...
    <div x-data="{ selectedPosts: [], selectAll: false, showFilters: false }" class="space-y-6">
        ...
        <div class="bg-white rounded-2xl shadow-lg p-6 border border-slate-100">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <a href="<?php echo e(route('admin.posts.create')); ?>" class="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-2 rounded-xl hover:shadow-lg transition-all duration-200 flex items-center gap-2 font-medium">
            ...
            <div x-show="showFilters" x-transition class="mt-4 pt-4 border-t border-slate-200">
                <form method="GET" action="<?php echo e(route('admin.posts.index')); ?>" class="flex flex-wrap items-end gap-3">
                    <div class="flex-1 min-w-[150px]">
                        ...
                        <select name="type" class="w-full px-2 py-1.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-xs">
                            ...
                            <option value="job" <?php echo e(request('type') == 'job' ? 'selected' : ''); ?>>Jobs</option>
                            <option value="result" <?php echo e(request('type') == 'result' ? 'selected' : ''); ?>>Results</option>
                            <option value="admit_card" <?php echo e(request('type') == 'admit_card' ? 'selected' : ''); ?>>Admit Cards</option>
                            <option value="answer_key" <?php echo e(request('type') == 'answer_key' ? 'selected' : ''); ?>>Answer Keys</option>
                            <option value="syllabus" <?php echo e(request('type') == 'syllabus' ? 'selected' : ''); ?>>Syllabus</option>
                            <option value="blog" <?php echo e(request('type') == 'blog' ? 'selected' : ''); ?>>Blogs</option>
                        </select>
                    </div>
                    ...
                    <div class="flex-1 min-w-[180px]">
                        ...
                        <input type="text" name="search" value="<?php echo e(request('search')); ?>" placeholder="Search posts..." class="w-full px-2 py-1.5 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 text-xs">
                    </div>
                    <div class="flex gap-2">
                        ...
                        <a href="<?php echo e(route('admin.posts.index')); ?>" class="bg-slate-200 text-slate-700 px-4 py-1.5 rounded-lg hover:bg-slate-300 transition text-xs font-medium whitespace-nowrap">
        ...
        <div x-show="selectedPosts.length > 0" x-transition class="bg-gradient-to-r from-blue-50 to-purple-50 border border-blue-200 rounded-2xl p-4 shadow-sm">
            <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
                ...
                <form action="<?php echo e(route('admin.posts.bulk-action')); ?>" method="POST" class="flex flex-wrap gap-2" id="bulkActionForm">
                    <?php echo csrf_field(); ?>
                        ...
                        <thead class="bg-gradient-to-r from-slate-50 to-slate-100 border-b border-slate-200">
                            <tr>
                                <th class="px-6 py-4 text-left">
                                    <input type="checkbox" @change="selectAll = !selectAll; selectedPosts = selectAll ? <?php echo e(json_encode($posts->pluck('id')->toArray())); ?> : []" class="rounded border-slate-300 text-blue-600 focus:ring-blue-500">
                                </th>
                        ...
                        <tbody class="divide-y divide-slate-100">
                            <?php $__currentLoopData = $posts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $post): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <tr class="hover:bg-slate-50 transition-all duration-200">
                                    <td class="px-6 py-4">
                                        <input type="checkbox" :value="<?php echo e($post->id); ?>" 
                                            @change="selectedPosts.includes(<?php echo e($post->id); ?>) ? selectedPosts = selectedPosts.filter(id => id !== <?php echo e($post->id); ?>) : selectedPosts.push(<?php echo e($post->id); ?>)"
                                            :checked="selectedPosts.includes(<?php echo e($post->id); ?>)"
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-3">
                                            ...
                                            <div>
                                                <p class="font-semibold text-slate-800 mb-1"><?php echo e(Str::limit($post->title, 40)); ?></p>
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="space-y-2">
                                            <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium
                                                <?php switch($post->type):
                                                    case ('job'): ?> bg-blue-100 text-blue-800 <?php break; ?>
                                                    <?php case ('result'): ?> bg-emerald-100 text-emerald-800 <?php break; ?>
                                                    <?php case ('admit_card'): ?> bg-orange-100 text-orange-800 <?php break; ?>
                                                    <?php case ('answer_key'): ?> bg-purple-100 text-purple-800 <?php break; ?>
                                                    <?php case ('syllabus'): ?> bg-indigo-100 text-indigo-800 <?php break; ?>
                                                    <?php case ('blog'): ?> bg-pink-100 text-pink-800 <?php break; ?>
                                                    <?php default: ?> bg-slate-100 text-slate-800
                                                <?php endswitch; ?>
                                            ">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-4 text-sm text-slate-600">
                                            <div class="flex items-center gap-1">
                                                ...
                                                <span class="font-medium"><?php echo e(number_format($post->view_count)); ?></span>
                                            </div>
                                        </div>
                                    </td>
                                    <td class="px-6 py-4">
                                        <div class="flex items-center gap-2">
                                            <a href="<?php echo e(route('admin.posts.edit', $post)); ?>" class="bg-blue-100 hover:bg-blue-200 text-blue-700 p-2 rounded-lg transition-all duration-200" title="Edit">
                                                <i class="fas fa-edit text-sm"></i>
                                            </a>
                                            <a href="<?php echo e(route('posts.show', [$post->type, $post])); ?>" target="_blank" class="bg-emerald-100 hover:bg-emerald-200 text-emerald-700 p-2 rounded-lg transition-all duration-200" title="View">
                                                <i class="fas fa-external-link-alt text-sm"></i>
                                            </a>
                                            <form action="<?php echo e(route('admin.posts.destroy', $post)); ?>" method="POST" class="inline" onsubmit="return confirm('Are you sure you want to delete this post?')">
                                                <?php echo csrf_field(); ?>
        ...
        <?php else: ?>
            ...
            <div class="bg-white rounded-2xl shadow-lg p-12 text-center border border-slate-100">
                ...
                <a href="<?php echo e(route('admin.posts.create')); ?>" class="bg-gradient-to-r from-blue-600 to-purple-600 text-white px-6 py-3 rounded-xl hover:shadow-lg transition-all duration-200 inline-flex items-center gap-2 font-medium">
                    <i class="fas fa-plus"></i>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp\htdocs\job\govt-job-portal-new\resources\views/admin/posts/index.blade.php ENDPATH**/ ?>