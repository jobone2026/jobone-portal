<div class="bg-white rounded-lg shadow-md p-6" x-data="seoAnalyzer()">
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
        <div>
            ...
            <select id="type" name="type" required class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-600">
                ...
                <option value="job" <?php echo e(old('type', $post->type ?? '') === 'job' ? 'selected' : ''); ?>>Job</option>
                <option value="admit_card" <?php echo e(old('type', $post->type ?? '') === 'admit_card' ? 'selected' : ''); ?>>Admit Card</option>
                <option value="syllabus" <?php echo e(old('type', $post->type ?? '') === 'syllabus' ? 'selected' : ''); ?>>Syllabus</option>
                <option value="result" <?php echo e(old('type', $post->type ?? '') === 'result' ? 'selected' : ''); ?>>Result</option>
                <option value="answer_key" <?php echo e(old('type', $post->type ?? '') === 'answer_key' ? 'selected' : ''); ?>>Answer Key</option>
        ...
        <div>
            ...
            <select id="state_id" name="state_id" class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-600">
                ...
                <?php $__currentLoopData = $states; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $state): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <option value="<?php echo e($state->id); ?>" <?php echo e(old('state_id', $post->state_id ?? '') == $state->id ? 'selected' : ''); ?>><?php echo e($state->name); ?></option>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </select>
        </div>
    </div>
    ...
    <div class="mb-6">
        ...
        <textarea id="content" name="content" rows="10" required 
                  class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-600"
                  x-model="content" @input="analyze()"><?php echo e(old('content', $post->content ?? '')); ?></textarea>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div>
            <label for="meta_title" class="block text-gray-700 font-bold mb-2">Meta Title (max 60 chars)</label>
            <input type="text" id="meta_title" name="meta_title" maxlength="60" value="<?php echo e(old('meta_title', $post->meta_title ?? '')); ?>" 
                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-600"
        ...
        <div>
            ...
            <input type="text" id="meta_description" name="meta_description" maxlength="160" value="<?php echo e(old('meta_description', $post->meta_description ?? '')); ?>" 
                   class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-600"
    ...
    <div class="mb-6">
        ...
        <input type="text" id="meta_keywords" name="meta_keywords" value="<?php echo e(old('meta_keywords', $post->meta_keywords ?? '')); ?>" 
               class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:border-blue-600"
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
        <div class="flex items-center">
            <input type="checkbox" id="is_featured" name="is_featured" value="1" <?php echo e(old('is_featured', $post->is_featured ?? false) ? 'checked' : ''); ?> class="mr-2">
            <label for="is_featured" class="text-gray-700 font-bold">Featured Post</label>
        </div>
        
        <div class="flex items-center">
            <input type="checkbox" id="is_published" name="is_published" value="1" <?php echo e(old('is_published', $post->is_published ?? false) ? 'checked' : ''); ?> class="mr-2">
    ...
    <div class="flex gap-4">
        ...
        <a href="<?php echo e(route('admin.posts.index')); ?>" class="bg-gray-400 text-white px-6 py-2 rounded-lg hover:bg-gray-500">
            Cancel
        </a>
    </div>
</div>
...
function seoAnalyzer() {
    return {
        title: '<?php echo e(old('title', $post->title ?? '')); ?>',
        content: `<?php echo e(old('content', $post->content ?? '')); ?>`,
        metaTitle: '<?php echo e(old('meta_title', $post->meta_title ?? '')); ?>',
        metaDescription: '<?php echo e(old('meta_description', $post->meta_description ?? '')); ?>',
        metaKeywords: '<?php echo e(old('meta_keywords', $post->meta_keywords ?? '')); ?>',
...
<?php /**PATH C:\xampp\htdocs\job\govt-job-portal-new\resources\views/admin/posts/form.blade.php ENDPATH**/ ?>